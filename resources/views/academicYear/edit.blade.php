        ...
        <div class="widget-header">
            <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <h4>تعديل معلومات السنة الدراسية - {{$academicYear->name}}</h4>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area-normal">
            <form action="{{ route('academic-years.update',['academic_year'=>$academicYear]) }}" method="POST">
                @method('PUT')
                @csrf
                    ...
                    <div class="col-6 col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror"
                                ...
                                <option {{old('status',$academicYear->status) == '0' ? 'selected' : '' }} value="0">غير فعال</option>
                            </select>
                            @error('status')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
